...
export default function BpmnTreeWalker(handler, translate) {
  ...
  function registerSemantic(element, context, definitions, plane) {
    ...
    for (const key in handlers) {
      if (is(element, key)) {
        handlers[key](element, context, definitions);
        valid = true;
        break;
      }
    }
  ...
  var handlers = {
    'bpmn:Process': function(element, context, definitions) {
      handleProcess(element, context);
    },
    'bpmn:SubProcess': function(element, context, definitions) {
      handleSubProcess(element, context);
    },
    'bpmn:Collaboration': function(element, context, definitions) {
      handleCollaboration(element, context);
  ...
  function handleSemantic(definitions, diagram) {
    ...
    var ctx = visit(rootElement, plane);

    registerSemantic(rootElement, ctx, definitions, plane);
  }
